...
from apps.studio.serializers import ServiceSerializer
...
class BookingSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
        ...
        if booking_type == Booking.BookingType.CONSULTATION:
            slot = attrs.get("consultation_slot")
            if not slot:
                raise serializers.ValidationError({"consultation_slot": "Required for consultation bookings."})
            if slot.status != ConsultationSlot.Status.AVAILABLE:
                raise serializers.ValidationError({"consultation_slot": "This slot is no longer available."})
        elif booking_type == Booking.BookingType.SESSION:
            block = attrs.get("session_block")
            hours = attrs.get("session_hours")
            if not block:
                raise serializers.ValidationError({"session_block": "Required for session bookings."})
            if not hours:
                raise serializers.ValidationError({"session_hours": "Required for session bookings."})
            if float(hours) > block.available_hours:
                raise serializers.ValidationError(
                    {"session_hours": f"Only {block.available_hours:.1f} hours available."}
                )
        return attrs
